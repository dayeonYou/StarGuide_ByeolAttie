...
#endif
public class Mouse_EyeTrackingScript : MonoBehaviour
{
    public pause pause;
    ...
    public GameObject GazePoint1;
    public GameObject GazePoint2;
        ...
        if (isTracking)
        {
            if (!pause.Startstate)
            {
                if (isNewGaze)
                {
                    ...
                    GazePoint1.GetComponent<RectTransform>().anchoredPosition = new Vector2(gazeX * overlayCanvasSizeDelta.x, gazeY * overlayCanvasSizeDelta.y);
                    isNewGaze = false;
                }
                GazePoint1.SetActive(true);
                GazePoint2.SetActive(false);
            }
            else
            {
                if (isNewGaze)
                {
                    Vector2 overlayCanvasSizeDelta = OverlayCanvas.GetComponent<RectTransform>().sizeDelta;
                    GazePoint2.GetComponent<RectTransform>().anchoredPosition = new Vector2(gazeX * overlayCanvasSizeDelta.x, gazeY * overlayCanvasSizeDelta.y);
                    isNewGaze = false;
                }
                GazePoint2.SetActive(true);
                GazePoint1.SetActive(false);
            }
        }
        else
        {
